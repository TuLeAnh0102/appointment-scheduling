...
function ThongKeTheoLuotVanTaiPage() {
  ...
  const loadData = () => {
    ...
    if (!loading) {
      setLoading(true);
      reportService.thongkeLuotVanTai(chotDichTe, loaiPhuongTien, moment(startDate).format(FORMAT_DATETIME), moment(endDate).format(FORMAT_DATETIME))
        .then((res) => {
...
export default ThongKeTheoLuotVanTaiPage;
